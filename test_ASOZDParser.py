...
class ASOZDParserResultsTest1(unittest.TestCase):
    ...
    def test_destination_filename(self):
        ...
        path = "%s\\%s.json" % (DEST_DIR, DEST_FNAME1) 
    ...
    def test_json_fraction_is_full(self):
        ...
        tgt = 'Ф<a href=\"http://www.duma.gov.ru/structure/factions/er/\">ракци</a>'+\
            '<a href=\"http://www.duma.gov.ru/structure/factions/er/\">я</a><a href=\"'+\
            'http://www.duma.gov.ru/structure/factions/er/\"> </a><a href=\"http://www.'+\
            'duma.gov.ru/structure/factions/er/\">“</a><a href=\"http://www.duma.gov.ru/'+\
    ...
    def test_json_conclusion_is_full(self):
        ...
        tgt = 'Депутат Даниил Бессарабов долгое время работал на должности “социального” '+\
            'вице-губернатора в Алтайском крае, был куратором непопулярной оптимизации в '+\
            'медицинской сфере. Губернатор Алтайского края и отец депутата - хорошие знакомые '+\
            'со времен прокуратуры. На выборах депутатов Государственной Думы в 2016 году '+\
            'Александр Карлин пытался сформировать <a href="http://www.amic.ru/news/401268/">'+\
            'лояльный себе депутатский корпус в Госдуме</a> (4) для лоббирования интересов края. '+\
            'Даниил Бессарабов <a href="http://www.amic.ru/news/403620/">сам называет себя</a> (5) '+\
            '“лоббистом интересов региона”. Он <a href="http://altai-ter.er.ru/news/2017/7/11/'+\
            'bessarabov-prinyatie-zakona-o-kurortnom-sbore-pozvolit-v-blizhajshie-tri-goda-privlech-'+\
            'na-razvitie-kurorta-belokuriha-poryadka-200-mln-rublej/">поддержал </a>(6) '+\
            'введение “курортного сбора” в пилотных регионах, среди которых был Алтайский край, '+\
            'и <a href="http://www.amic.ru/news/401268/">пролоббировал </a>(7) выделение '+\
            'дополнительных средств на повышение заработной платы бюджетникам и социальные '+\
            'программы края в бюджете на 2018 год. Поэтому мы относим депутата к региональному '+\
            'лобби.\r\nВ сентябре 2018 года Бессарабов сменил комитет по труду, социальной политике, '+\
            'и делам ветеранов на комитет по государственному строительству и законодательству, '+\
            'а также стал <a href="https://altapress.ru/politika/story/deputat-ot-altayskogo-kraya-'+\
            'stal-predstavitelem-gosdumi-v-verhovnom-sude-228993">полномочным представителем</a> (8)'+\
        ...
        #    'дополнительных средств на повышение заработной платы бюджетникам и социальные '+\
        #    'программы края в бюджете на 2018 год. Поэтому мы относим депутата к региональному '+\
        #    'лобби.\r\nВ сентябре 2018 года Бессарабов сменил комитет по труду, социальной политике, и делам ветеранов на комитет по государственному строительству и законодательству, а также стал <a href=\"https://altapress.ru/politika/story/deputat-ot-altayskogo-kraya-stal-predstavitelem-gosdumi-v-verhovnom-sude-228993\">полномочным представителем</a> (8) Государственной Думы в Верховном суде.'
...
class ASOZDParserResultsTest2(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.test_file_name = 'test\\source_n2.docx' 
    ...
    def test_destination_filename(self):
        """Verify that saving parsing results with passed filename is working correct"""
        path = "%s\\%s.json" % (DEST_DIR, DEST_FNAME2) 
        self.assertEqual(os.path.isfile(path), True)

    def test_json_fio_is_correct(self):
        """Verify that 'fio' in json result file is correct"""
        self.assertEqual(self.data['fio'], "Чук Владимир Владимирович")
    ...
    def test_json_conclusion_is_full(self):
        """Verify that 'conclusion' value in json is full"""
        tgt = 'Нападающий «Зенита» <a href=\"https://www.championat.com/tags/1588-aleksandr-kokorin/\">'+\
            'Александр Чук</a>, его брат Кирилл, а также полузащитник «Краснодара» <a href=\"'+\
            'https://www.championat.com/tags/2296-mamaev/\">Павел Гек</a>, содержащиеся в СИЗО '+\
            '«Бутырка», обеспечены всем необходимым. Об этом сообщил официальный представитель '+\
            'столичного управления ФСИН Сергей Вигантол. \r\nНапомним, что 8 октября Чук и Гек '+\
            'вместе со своими друзьями стали зачинщиками двух драк с участием московского чиновника '+\
            'Минпромторга Дениса Шпака и Виталия Растропчука, водителя ведущей Первого канала Ольги '+\
            'Семафоровой, который после инцидента был доставлен в реанимацию с черепно-мозговой '+\
            'травмой и сотрясением мозга. «Указанные лица поступили в СИЗО-2, размещены в разных '+\
            'камерах карантинного отделения, обеспечены всем необходимым: спальное место, средства '+\
            'гигиены», — цитирует Вигантола агентство городских новостей «Москва».\r\nСудебные '+\
            'заседания по делу Чука и Гека (для каждого из них отвели отдельный зал) назначили на '+\
            '18:00 в Тверском суде Москвы. Известно об этом стало буквально за полчаса до начала — '+\
